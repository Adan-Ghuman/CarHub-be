...
try {
    ...
    $service_check = $conn->prepare("SELECT id, workshop_id, service_name FROM workshop_services WHERE id = ?");
    ...
    $booking_check = $conn->prepare("SELECT COUNT(*) as booking_count FROM workshop_bookings WHERE service_id = ? AND status IN ('pending', 'confirmed')");
    ...
    $delete_sql = "UPDATE workshop_services SET is_active = 0, updated_at = NOW() WHERE id = ?";
